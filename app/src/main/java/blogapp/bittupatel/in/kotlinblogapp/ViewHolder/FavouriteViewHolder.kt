...
class FavouriteViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    init {
        itemView.setOnClickListener {
            val intent = Intent(itemView.context, FavPostDetail::class.java)

//            intent.putExtra(POST_IMAGE_KEY, post?.thumbnail)
            intent.putExtra(POST_TITLE_KEY, post?.title)
